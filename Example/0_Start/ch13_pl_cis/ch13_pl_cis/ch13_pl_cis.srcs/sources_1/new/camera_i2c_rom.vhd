    ...
    if(clk'event and clk = '1')then
        if(SW_1 = '1')then
            case i2c_rom_address is
                ...
                -- FORMAT MUX CONTROL : ISP RGB
                when    x"6A"   =>  i2c_rom_data        <=  x"78501F01";
                -- DITHER CTRL : RGB565/555
                when    x"6B"   =>  i2c_rom_data        <=  x"7850202A";    
                -- FORMAT CONTROL RG : RGB(r[4:0],g[5:3],g[2:0],b[4:0])
                when    x"6C"   =>  i2c_rom_data        <=  x"78430061";    
                -- TIMING TC REG20 : Sensor vflip
                when    x"6D"   =>  i2c_rom_data        <=  x"78382002";    
                -- TIMING TC REG21 : Sensor mirror
                when    x"6E"   =>  i2c_rom_data        <=  x"78382102";    
